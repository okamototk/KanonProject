...
public final class AssignmentDetail implements Schedule, HasCalendar, Cloneable, Serializable, ContourTypes {// , Schedule {
	...
	private boolean timesheetComplete = false;
//	private boolean timesheetClosed = false;
	/**
...
//			System.out.println("delay " + new java.util.Date(getStart()) + new java.util.Date(getTaskStart()));
	...
	void adjustRemainingDuration(long newRemainingDuration) {
		...
		workContour = workContour.adjustDuration(d, actualDuration, getUnits()); // allow a personal contour to adjust itself
 ...
 */
	public long getStart() {
		...
		if (delay > 0)
		{
			// JGao -- 7/20/2009 Changed the logic for calculating start if there is delay.
			// It seems that delay is always caused by assignment started later. However the delay is
			// calculated using effective work calendar. So the starting point needs to be adjusted
			// first and then adding the daly in order to correctly calculate the assignment start.
			WorkCalendar ewc = getEffectiveWorkCalendar();
//			s = ewc.add(s, 0, false);
			s = ewc.add(s, delay, false); // use later time
//			s = ewc.adjustInsideCalendar( s, false );
		}
...
//		// JGao 9/11/2009 Short circuit the no completion case. Otherwise, there could be rounding problem.
...
//		}
		...
		setDuration(getDuration() + (remainingDuration - oldRemaining));
//
//
...
//			}
//			long actualDuration = getWorkContour().calcActualTotalBucketDuration(0);
...
//			long actualDuration = 0;
			...
			*/
			...
			a.timesheetComplete = timesheetComplete;
//			a.timesheetClosed = timesheetClosed;
			return a;
//		}
//		catch (CloneNotSupportedException e) {
...
//		}
	...
	public void addCalendarTime(long start) {
		if (actualExceptionsCalendar == null) {
			WorkCalendar base = getEffectiveWorkCalendar();
			actualExceptionsCalendar = CalendarService.getInstance().getStandardBasedInstance();
			try {
		...
		actualExceptionsCalendar.addCalendarTime(start);
	}
	...
	public boolean isTimesheetComplete() {
		return timesheetComplete;
	}
	public void setTimesheetComplete(boolean timesheetComplete) {
		this.timesheetComplete = timesheetComplete;
	}
	...
	public boolean fieldHideTimesheetClosed(FieldContext fieldContext) {
		return !getTask().getProject().isTimesheetClosed();
	}
	public boolean isReadOnlyTimesheetClosed(FieldContext fieldContext) {
		return !getTask().getProject().isTimesheetAssociated() || (Duration.millis(duration) > 0L && !isComplete());
	}

	public boolean isActualsProtected() {
		return getTask().getProject().isActualsProtected();
	}
