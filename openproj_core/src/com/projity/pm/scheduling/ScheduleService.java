...
//		if (oldValue==null) oldValue=new Long(schedule.getActualStart());
		...
		try {
			completedField.setValue(schedule,eventSource,value,null,options);
			if (options.getChange()!=null&&undoableEditSupport!=null&&!(eventSource instanceof UndoableEdit)){
				FieldSetOptions undoOptions=new FieldSetOptions();
				undoOptions.setRawProperties(true);
				undoableEditSupport.postEdit(new FieldEdit(completedField,schedule,options.getChange().getNewValue(),options.getChange().getOldValue(),eventSource,null,undoOptions));
			}
