...
public class Project implements Document, BelongsToDocument, HasKey, HasPriority, MutableHasStartAndEnd, ProjectSpecificFields, HasNotes, HasBaseCalendar, HasCalendar, NodeModelDataFactory, HierarchyListener, HasTimeDistributedData, TimeDistributedFields, EarnedValueValues, EarnedValueFields, DataObject, HasSentinels, BaselineScheduleFields, Schedule,UpdatesFromTimesheet, HasExtraFields, HasExpenseType, SavableToWorkspace {
	...
	long lastTimeTimesheetPolled = 0; // NOT transient

	long persistedId = 0; // used to see if copied from template
	boolean wasImported = false; //used to see if it originated from an MSP or POD import
	...
	private transient boolean actualsProtected = false;
//	private transient TaskCompletionOptions taskCompletionOption = TaskCompletionOptions.PERCENT_COMPLETE;
	private transient boolean completionFromTimesheet = false;

	private transient boolean timesheetClosed = false;
	...
	private Project(boolean local) {
		...
		calendarOption = (wc.getName() == "Company") ? CalendarOption.getInstance() : CalendarOption.getDefaultInstance();
	}
 ...
 */
	...
	public void setLocalParent(Node childNode, Node parentNode) {
		if (childNode.getParent()==parentNode) return;
		if (parentNode==null&&Environment.isAddSummaryTask()) parentNode=getvirtualTaskRoot();
...
//		newOne.initializeDates();
	...
	public NormalTask createScriptedTask( boolean rootLevel,boolean undo) {
		NormalTask task = newStandaloneNormalTaskInstance();
		task.markTaskAsNeedingRecalculation();
		connectScriptedTask(task, rootLevel,undo);
		return task;
	}
	public Node createScriptedNode( boolean rootLevel,boolean undo) {
		NormalTask task = newStandaloneNormalTaskInstance();
		task.markTaskAsNeedingRecalculation();
		return connectScriptedTask(task, rootLevel, undo);
	}
	private Node connectScriptedTask(NormalTask task, boolean rootLevel, boolean undo){
		Node child = NodeFactory.getInstance().createNode(task);
		NodeModel model=getTaskModel();
		Node parent = (Node) model.getRoot();
		List children=model.getChildren(parent);
		boolean foundSummary=false;
		if (children!=null&&!rootLevel&&Environment.isAddSummaryTask())
		for (Iterator i=children.iterator();i.hasNext();){
			Node n=(Node)i.next();
			if (!(n.getImpl() instanceof Task)) continue;
			Task t=(Task)n.getImpl();
			if (t.isRoot()){
				foundSummary=true;
				parent=n;
				break;
		...
		if (foundSummary) children=parent.getChildren();
		int count = children==null?0:children.size();//getTaskModel().getChildCount(parent);
	...
	protected boolean summaryTaskEnabled=false;

	public boolean isSummaryTaskEnabled() {
		return summaryTaskEnabled;//getTaskModel().isSummaryNodeEnabled();
	}

	public void setSummaryTaskEnabled(boolean summaryTaskEnabled) {
		this.summaryTaskEnabled=summaryTaskEnabled;
	}
	...
	public Task getSummaryTask(){
		return (Task) getSummaryTaskNode().getImpl();
	}
...
//	}
	...
	public boolean remove(Node toRemove){
		...
		model.remove(toRemove,NodeModel.NORMAL);
		return true;
	}


	public boolean paste(Node parent,List<Node> nodes,int position){
		if (nodes==null||nodes.size()==0) return false;
		NodeModel model=getTaskOutline();
		if (parent==null) return false;
		model.paste(parent, nodes, position, NodeModel.NORMAL);
		return true;
	}
	public List cut(List<Node> nodes){
		if (nodes==null||nodes.size()==0) return null;
		NodeModel model=getTaskOutline();
		List result=model.cut(nodes,NodeModel.NORMAL);
		return result;
	}
	public List copy(List<Node> nodes){
		if (nodes==null||nodes.size()==0) return null;
		NodeModel model=getTaskOutline();
		List result=model.copy(nodes,NodeModel.NORMAL);
		return result;
	}
	public boolean move(Node parent,List<Node> nodes,int position){
		if (nodes==null||nodes.size()==0) return false;
		NodeModel model=getTaskOutline();
		model.move(parent, nodes, position, NodeModel.NORMAL);
		return true;
	}
...
//		removeEndSentinelDependency(toRemove);
	...
	public void clearAllSnapshots() {
		for (int i = 0; i < Settings.NUM_ARRAY_BASELINES; i++) {
			clearSnapshot(new Integer(i),true,null,false);
		}
	}
...
//	}
	...
	public double getBaselineCost(int numBaseline, FieldContext fieldContext) {
		if (fieldContext != null && fieldContext.isHideNullValues() && !hasBaseline(numBaseline))
			return ClassUtils.NULL_DOUBLE;
		if (Environment.isNoPodServer()) {
			Field f = Configuration.getFieldFromId("Field.baseline" + (numBaseline == 0 ? "" : (""+numBaseline)) + "Cost");
			Money value = (Money) f.getValue(getSummaryTaskNode(),taskModel,fieldContext);
			if (value == null) {
		...
		return baselineCost(FieldContext.start(fieldContext),FieldContext.end(fieldContext));	 // this is not correct
	}
	/* (non-Javadoc)
	...
	public long getBaselineWork(int numBaseline, FieldContext fieldContext) {
		...
		if (Environment.isNoPodServer()) {
			Field f = Configuration.getFieldFromId("Field.baseline" + (numBaseline == 0 ? "" : (""+numBaseline)) + "Work");
			Work value = (Work) f.getValue(getSummaryTaskNode(),taskModel,fieldContext);
			if (value == null) {
				if (fieldContext != null && fieldContext.isHideNullValues())
					return ClassUtils.NULL_LONG;
				return 0L;
		...
		return baselineWork(FieldContext.start(fieldContext),FieldContext.end(fieldContext));	// this is not correct
	}

	private boolean hasBaseline(int numBaseline) { // see if any task has a baseline
		Integer snapshotId = Integer.valueOf(numBaseline);
		Iterator i = tasks.iterator();
		while (i.hasNext()) {
			Task t = (Task)i.next();
			if (t.getSnapshot(snapshotId) != null)
				return true;
		}
		return false;
	}
...
*/
	...
	public boolean equals(Object obj){
	    if (obj instanceof DataObject){
	    	if (Environment.isNoPodServer()) {
	    		return getUniqueId() == (((DataObject)obj).getUniqueId());
	    	}
...
//	}
	...
	public long getBaselineDuration(int numBaseline, FieldContext fieldContext) {
	...
	public long getActualDuration() {
		...
		if (stop == 0)
			return 0;
		long s = Environment.isNoPodServer() ? getEarliestStartingTaskOrStart() : getStartDate();
	...
	public final void setGroupDirty(boolean isGroupDirty) {
		...
		if (undoController!=null) undoController.updateDirty();
		if (old!=isGroupDirty){
...
//	}
	...
	public boolean renumberProjectOnly(long newUniqueId){
		return hasKey.renumber(false, newUniqueId);
	}
...
//	}
	...
	protected transient Set<Long> initialTaskIds;
	protected transient Set<DependencyKey> initialLinkIds;
	protected transient Set<AssignmentKey> initialAssignmentIds;
	protected transient Set<Long> newTaskIds;
	protected transient Set<DependencyKey> newLinkIds;
	protected transient Set<AssignmentKey> newAssignmentIds;

	public Set<DependencyKey> getInitialLinkIds() {
		return initialLinkIds;
	}

	public void setInitialLinkIds(Set<DependencyKey> initialLinkIds) {
		this.initialLinkIds = initialLinkIds;
	}
	...
	public void setInitialTaskIds(Set<Long> initialTaskIds) {
	...
	public void setNewLinkIds(Set<DependencyKey> newLinkIds) {
		this.newLinkIds = newLinkIds;
	}
	...
	public void setNewTaskIds(Set<Long> newTaskIds) {
		this.newTaskIds = newTaskIds;
	}

	public Set<AssignmentKey> getInitialAssignmentIds() {
		return initialAssignmentIds;
	}

	public void setInitialAssignmentIds(Set<AssignmentKey> initialAssignmentIds) {
		this.initialAssignmentIds = initialAssignmentIds;
	}
	...
	public void setNewAssignmentIds(Set<AssignmentKey> newAssignmentIds) {
		this.newAssignmentIds = newAssignmentIds;
	}


	public Set<AssignmentKey> generateInitialAssignmentIdsIncludingExceptions(){
		Set<AssignmentKey> all=new HashSet<AssignmentKey>();
		if (initialAssignmentIds!=null) all.addAll(initialAssignmentIds);
		forObjects(null, all, null, null, new AssignmentPredicate(){
			public boolean evaluate(Assignment assignment, int snapshotId) {
	...
	public void validateNewTaskAndAssignments(){
		if (newTaskIds!=null&&newTaskIds.size()>0){
			if (initialTaskIds==null) initialTaskIds=new HashSet<Long>();
			else initialTaskIds.clear();
			initialTaskIds.addAll(newTaskIds);
			newTaskIds=null;
		}
		if (newLinkIds!=null&&newLinkIds.size()>0){
			if (initialLinkIds==null) initialLinkIds=new HashSet<DependencyKey>();
			else initialLinkIds.clear();
			initialLinkIds.addAll(newLinkIds);
			newLinkIds=null;
		}
		if (newAssignmentIds!=null&&newAssignmentIds.size()>0){
			if (initialAssignmentIds==null) initialAssignmentIds=new HashSet<AssignmentKey>();
			else initialAssignmentIds.clear();
			initialAssignmentIds.addAll(newAssignmentIds);
			newAssignmentIds=null;
		}
	}

    public static interface AssignmentClosure{
    	public void execute(Assignment assignment,int snapshotId);
    }
    public static void forAssignments(NormalTask task,AssignmentClosure c){
	...
	public void forObjects(final Set<Long> taskIds,final Set<AssignmentKey> assignmentIds,final Set<DependencyKey> linkIds,final Predicate taskFilter,final AssignmentPredicate assignmentFilter,final Predicate linkFilter){
		forTasks(new Closure(){
			public void execute(Object arg) {
				NormalTask task=(NormalTask)arg;
				if (taskFilter==null||taskFilter.evaluate(task)){
					if (taskIds!=null) taskIds.add(task.getUniqueId());
					if (assignmentIds!=null||assignmentFilter!=null)
			            forAssignments(task, new AssignmentClosure(){
			            	public void execute(Assignment assignment,int s){
			            		if ((assignmentFilter==null||assignmentFilter.evaluate(assignment,s))&&
			            				assignmentIds!=null)
			            			assignmentIds.add(new AssignmentKey(assignment.getTask().getUniqueId(),assignment.getResource().getUniqueId(),s));
			            	}
			            });
					if (linkIds!=null||linkFilter!=null)
						for (Iterator i=task.getPredecessorList().iterator();i.hasNext();){
							Dependency dependency=(Dependency)i.next();
		            		if ((linkFilter==null||linkFilter.evaluate(dependency))&&
		            				linkIds!=null)
		            			linkIds.add(new DependencyKey(dependency.getPredecessorId(),dependency.getSuccessorId()/*,dependency.getExternalId()*/));
						}
				}
			}
		});
	...
	public void setNewIds(){
		newTaskIds=new HashSet<Long>();
		newLinkIds=new HashSet<DependencyKey>();
		newAssignmentIds=new HashSet<AssignmentKey>();
		forObjects(newTaskIds, newAssignmentIds, newLinkIds,null,null,null);
	}
	public void setInitialIds(){
		initialTaskIds=new HashSet<Long>();
		initialLinkIds=new HashSet<DependencyKey>();
		initialAssignmentIds=new HashSet<AssignmentKey>();
		forObjects(initialTaskIds, initialAssignmentIds, initialLinkIds,null,null,null);
	}
	...
	public boolean isTestImportOk() {
		...
		while (i.hasNext()) {
			if (!((NormalTask)i.next()).isTestImportOk())
	...
	public boolean isActualsProtected() {
		return isDatesPinned() || actualsProtected;
	}
	...
	public boolean isCompletionFromTimesheet() {
		return completionFromTimesheet;
	}

	public void setCompletionFromTimesheet(boolean completionFromTimesheet) {
		this.completionFromTimesheet = completionFromTimesheet;
	}
...
//	}
	...
	public boolean isReadOnlyPercentComplete(FieldContext context) {
		return true;
	}
...
//	}
	...
	private NormalTask findFromInvestmentMilestone(String id) {
		for (NormalTask t : (Collection<NormalTask>)tasks) {
			if (t.getInvestmentMilestone() == id)
				return t;
		}
		return null;
	}

	transient HashMap<String,String> investmentMilestones;
	public HashMap<String, String> getInvestmentMilestones() {
		return investmentMilestones;
	}
	public String[] availableInvestmentMilestones() {
		ArrayList list = new ArrayList();
		for (String i : investmentMilestones.keySet()) {
...
//	}
	...
	transient String checkedOutUser;
	transient Calendar checkedOutDate;
	transient String publishedUser;
	transient Calendar publishedDate;
	transient int checkedOutUserId = 0;
	transient boolean mspAssociated;
	public int getCheckedOutUserId() {
		return checkedOutUserId;
	}

	public void setCheckedOutUserId(int checkedOutUserId) {
		this.checkedOutUserId = checkedOutUserId;
		System.out.println("Project checked out user id = " + checkedOutUserId + " locked by self " + this.isLocked());
	}
	...
	public void setCheckedOutDate(Calendar checkedOutDate) {
...
//		System.out.println("=========== set publish date " + (publishedDate == null ? null : publishedDate.getTime()));
...
//	}
	...
	public String getPublishInfo() {
		return "Locked=" + isLocked() + " published date = " + (getPublishedDate() == null ? null : getPublishedDate().getTime()) + " by " + getPublishedUser() + " checked out date " + (getCheckedOutDate() == null ? null : getCheckedOutDate().getTime()) + " by " + getCheckedOutUser();
	}
	public String getPublishInfoHtml() {
		//System.out.println(Messages.getString("Text.LockedTooltip"));

		String result =
		"<html>Published date:" + (getPublishedDate() == null ? null : getPublishedDate().getTime()) +
		"<br>by " + getPublishedUser();
		if (getCheckedOutUser() != null)
			result += "<br>Checked out date " + (getCheckedOutDate() == null ? null : getCheckedOutDate().getTime()) +
					"<br>by " + getCheckedOutUser();
		result +="</html>";
		return result;
	}

	private transient ProjectPermission projectPermission =null;
	...
	public void setNoLongerLocked(boolean noLongerLocked) {
		this.noLongerLocked = noLongerLocked;
	}
	...
	//	System.out.println("mspAssociated=true  for testing!!!!");
	//	mspAssociated = true;
		return mspAssociated;
	}
	...
	public void setMspAssociated(boolean mspAssociated) {
		this.mspAssociated = mspAssociated;
	}
	...
	public boolean isReadOnlyCost(FieldContext fieldContext) {
		return true;
	}
	public void setCost(double cost, FieldContext fieldContext) {
	}
	public long getLastTimeTimesheetPolled() {
		return lastTimeTimesheetPolled;
	}

	public void setLastTimeTimesheetPolled(long lastTimeTimesheetPolled) {
		this.lastTimeTimesheetPolled = lastTimeTimesheetPolled;
	}
	...
	public boolean isTimesheetClosed() {
		return timesheetClosed;
	}
	public void setTimesheetClosed(boolean timesheetClosed) { // this means something different for project than for task
	...
	public boolean isTimesheetComplete() {
		Iterator i = tasks.iterator();
		while (i.hasNext()) {
			if (!((NormalTask)i.next()).isTimesheetComplete()); // really should just do root task
			return false;
		}
		return true;
	}

	public void setTimesheetComplete(boolean timesheetComplete) {
		Iterator i = tasks.iterator();
		while (i.hasNext()) {
			((NormalTask)i.next()).setTimesheetComplete(timesheetComplete); // really should just do root task
		}
	}
	public boolean fieldHideTimesheetClosed(FieldContext fieldContext) {
		return !isTimesheetClosed();
	}

	public boolean isReadOnlyActualDuration(FieldContext fieldContext) {
		return true;
	}
	public boolean isReadOnlyActualFinish(FieldContext fieldContext) {
		return true;
	}
	public boolean isReadOnlyActualStart(FieldContext fieldContext) {
		return true;
	}
	public boolean isReadOnlyCompletedThrough(FieldContext context) {
		return true;
	}
	public boolean isReadOnlyComplete(FieldContext context) {
		return isReadOnlyPercentComplete(context);
	}

	public boolean isReadOnlyTimesheetClosed(FieldContext fieldContext) {
		return true;
	}
	...
	public void setCurrencyRateTable(CurrencyRateTable table) {
		CurrencyRateTable.setInstance(table); // it is global
	}

	public void setWasImported(boolean value) {
		wasImported = value;
	}
