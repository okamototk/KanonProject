...
public class CalendarDefinition implements WorkCalendar, Cloneable {
	...
	public WorkDay findExceptionDay(long date) {
		...
		while (i.hasNext()) {
			WorkDay day = (WorkDay) i.next();
			if (day.getStart() == date)
				return day;
		}
		return null;
	}
...
//					System.out.println("error");
	...
	public void serialiseCompact(ObjectOutputStream s) throws IOException{
		s.writeByte(version);
		s.writeLong(id);
		week.serialiseCompact(s);
		if (dayExceptions==null) s.writeInt(-1);
		else{
			s.writeInt(dayExceptions.size());
			for (Iterator i=dayExceptions.iterator();i.hasNext();){
				WorkDay d=(WorkDay)i.next();
				d.serialiseCompact(s);
			}
		}
	...
	public void deserialiseCompact(ObjectInputStream s) throws IOException, ClassNotFoundException{
		version=s.readByte();
		id=s.readLong();
		week=WorkWeek.deserialiseCompact(s);
		int dayExceptionsCount=s.readInt();
		//if (dayExceptionsCount==-1) dayExceptions=null;
		//else{
			//dayExceptions=new TreeSet();
			for (int i=0;i<dayExceptionsCount;i++){
				dayExceptions.add(WorkDay.deserialiseCompact(s));
			}
		//}
		exceptions = new WorkDay[dayExceptions.size()];
		dayExceptions.toArray(exceptions);
