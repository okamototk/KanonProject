...
public class FieldDictionary {
	...
	public void addField(Field field) {
		if (Configuration.getFieldFromId(field.getId()) != null) // if already exists
			return;
...
//		digester.addObjectCreate("*/fieldDictionary", "com.projity.configuration.FieldDictionary");
		...
		digester.addFactoryCreate("*/field", "com.projity.field.FieldFactory");
		digester.addSetProperties("*/field");
		digester.addSetNext("*/field", "addField", "com.projity.field.Field");
		...
		digester.addSetNext("*/field/choice", "setSelect", "com.projity.field.Select"); // attach to field
		...
		digester.addCallMethod("*/fieldDictionary/removeField","removeField", 1, new Class[] {String.class});
		digester.addCallParam("*/fieldDictionary/removeField",0);
	...
	private static void fieldsToHtmlTable(final StringBuffer result,String title,Collection fields) {
		...
		CollectionUtils.forAllDo(fields, new Closure() {
			public void execute(Object arg0) {
				...
				if (f.isCustom())
					return;
				tabbedStringToHtmlRow(result,f.getMetadataString(),false);
			}}
		);
		result.append("</table>");
		result.append("</p>");
	...
	public static void generateSpreadsheetDefinition(String fileName) {
	...
	private static void generateSpreadsheetDefinition(StringBuffer result, String name, String category, Collection fields) {
		result.append("<spreadsheet name=" +"\"" + name+"\" category=" +"\"" + category+"\">\n");
		...
		while (i.hasNext()) {
			...
			if (f.isCustom() || f.isExtra())
				continue;
			result.append("\t\t<column>" + f.getId() + "</column>\n");
		}
		result.append("\t</columns>\n");
