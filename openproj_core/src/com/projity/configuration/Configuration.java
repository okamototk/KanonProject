...
public class Configuration implements ProvidesDigesterEvents {
	...
	Set<String> removedMenu = new HashSet<String>();
...
//	}
	...
	public static final Field getFieldFromShortId(String id) {
		Field f =  getFieldFromId(Field.longName(id));
		if (f == null&&Environment.isNoPodServer())
			f =  getFieldFromId(Field.longCustomName(id));
		if (f == null)
			return getFieldFromId(id);
		return f;
	}
	...
	private void addGlobalDigesterEvents(Digester dg){
		...
		dg.addCallMethod("configuration/removeMenu","removeMenu",1,new Class[] {String.class});
		dg.addCallParam("configuration/removeMenu",0);

	}

	public void removeMenu(String id) {
		Configuration.getInstance().getRemovedMenu().add(id);
	}

	public boolean isRemovedMenu(String id) {
		return removedMenu.contains(id);
	}
	public void addDigesterEvents(Digester dg) {
		...
		if (digesterEventProviderString != null) {
			...
			for (String digesterProvider : digesterEventProviders) {
				try {
					Class.forName(digesterProvider).getMethod("addDigesterEvents", new Class[]{Digester.class}).invoke(null,new Object[] {dg});
				} catch (Exception e) {
