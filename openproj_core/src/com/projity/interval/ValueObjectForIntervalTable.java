...
public abstract class ValueObjectForIntervalTable implements NodeModelDataFactory, Serializable, Cloneable {
	...
	public ValueObjectForInterval add(long start, double value) {
		return addInterval(start,DateTime.getMaxDate().getTime(),value);

	}
	
	private long inRange(long val) {
		if (val < 0)
			return 0;
		if (val > DateTime.getMaxDate().getTime())
			return DateTime.getMaxDate().getTime();
		return val;
	}
	public ValueObjectForInterval addInterval(long start, long finish, double value) {
System.out.println("add interval " + DateTime.medString(start) + " " + DateTime.medString(finish) + " " + value + "  as millis" + value * WorkCalendar.MILLIS_IN_HOUR);

		if (start > DateTime.getMaxDate().getTime())
			return null;
		if (finish < 0)
			return null;
		
		start = inRange(start);
		finish = inRange(finish);
		if (start == finish)
			return null;
		try {
			if (valueObjects.isEmpty()) {
				ValueObjectForInterval initialEmpty = newValueObject(0);
				initialEmpty.setPrimaryValue(0.0D);
			}

			ValueObjectForInterval newOne = newValueObject(start);
			newOne.setPrimaryValue(value);
			if (finish != NO_VALUE) {
				ValueObjectForInterval empty = newValueObject(finish); // add blank space
				empty.setPrimaryValue(0.0D);
			}
			removeZeros();
			return newOne;
	...
	private void removeZeros() {
		...
		while (i.hasNext()) {
			ValueObjectForInterval v = (ValueObjectForInterval) i.next();
...
//	public void fireCreated(Object newlyCreated){}
	...
	public void serializeCompact(ObjectOutputStream s) throws IOException {
	    s.writeObject(name);
	    if (valueObjects==null){
	    	s.writeInt(-1);
	    	return;
	    }
	    s.writeInt(valueObjects.size());
	    for (Iterator i=valueObjects.iterator();i.hasNext();){
	    	ValueObjectForInterval interval=(ValueObjectForInterval)i.next();
	    	interval.serializeCompact(s,this);
	    }
	}
