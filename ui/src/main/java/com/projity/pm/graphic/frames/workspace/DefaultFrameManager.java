...
public class DefaultFrameManager implements FrameManager {
	...
	public void cleanUp(boolean justFrames) {
...
//		setLockStatus(frame.getProject().isLocked(),frame.getProject().isCheckedOutByOther(),frame.getProject().getCheckedOutUser());
		setLockStatus(frame.getProject().getLockStatus(),frame.getProject().getPublishInfoHtml(), frame.getProject().getCheckedOutUser());
...
//	public void setLockStatus(boolean locked, boolean lockedByOther,String tip) {
	...
	public void setLockStatus(Project.LockStatus status,String tip, String lockedBy) {
//		System.out.println("set lock status " + locked + " other " + lockedByOther +" " + tip);
		if (lock == null)
			return;
		if (status == Project.LockStatus.NONE) {
			lock.setVisible(false);
		} else {
			lock.setText("");
			ImageIcon icon = null;
			if (status == Project.LockStatus.UNLOCKED){
				icon = unlockIcon;
				lock.setText(Messages.getString("Text.unlocked") );
			}else if (status == Project.LockStatus.LOCKED){
				icon = lockIcon;
				lock.setText(Messages.getStringWithParam("Text.lockedBy",lockedBy) );
			}else if (status == Project.LockStatus.LOCKED_BY_OTHER){
				icon = otherLockIcon;
				lock.setText(Messages.getStringWithParam("Text.lockedBy",lockedBy) );
			}else if (status == Project.LockStatus.MSP){
				icon = mspIcon;
				lock.setText(Messages.getString("Text.lockedByMSP") );
			} else if (status == Project.LockStatus.NO_LONGER_LOCKED){
				icon = noLongerLockedIcon;
				lock.setText(Messages.getString("Text.noLongerLocked") );
			}
			lock.setIcon(icon);
			lock.setVisible(true);
			lock.setToolTipText(tip);
		}
	}
