...
public class TaskIndicatorsComponent extends IndicatorsComponent{
	...
	protected JLabel timesheetCompleted;
	protected JLabel taskButNotTimesheetCompleted;
	protected JLabel timesheetClosed;
	...
	public void init() {
		...
		try {
			timesheetCompleted = new JLabel(" ",IconManager.getIcon("indicator.timesheetCompleted"),  JLabel.RIGHT); //$NON-NLS-1$ //$NON-NLS-2$
			timesheetCompleted.setOpaque(false);
		} catch (MissingResourceException e) {}
		try {
			timesheetClosed = new JLabel(" ",IconManager.getIcon("indicator.timesheetClosed"),  JLabel.RIGHT); //$NON-NLS-1$ //$NON-NLS-2$
			timesheetClosed.setOpaque(false);
		} catch (MissingResourceException e) {}
		try {
			taskButNotTimesheetCompleted = new JLabel(" ",IconManager.getIcon("indicator.taskButNotTimesheetCompleted"),  JLabel.RIGHT); //$NON-NLS-1$ //$NON-NLS-2$
			taskButNotTimesheetCompleted.setOpaque(false);
		} catch (MissingResourceException e) {}
	...
	public void setIndicators(Object value,JComponent label,StringBuffer text,boolean isSelected, boolean hasFocus){
		...
		if (indicators instanceof NormalTask) {
			project = ((NormalTask)indicators).getProject();
			actualsProtected = ((NormalTask)indicators).isActualsProtected();
		} else if (indicators instanceof Assignment){
			project = ((Assignment)indicators).getProject();
			actualsProtected = ((Assignment)indicators).isActualsProtected();
		}			
		if (indicators instanceof Task && indicators.getWorkCalendar() != null) {
		...
		//TODO handle case of non-ts associated
		if (indicators.isComplete()) {
			if (!indicators.isTimesheetComplete() && actualsProtected) {
				setLook(taskButNotTimesheetCompleted,isSelected,hasFocus);
				label.add(taskButNotTimesheetCompleted);
				text.append(Messages.getString("TaskIndicatorsComponent.TaskCompleteButNotTimesheet") +"<br>"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		} else if (indicators.isTimesheetComplete()) {
			setLook(timesheetCompleted,isSelected,hasFocus);
			label.add(timesheetCompleted);
			text.append(Messages.getString("TaskIndicatorsComponent.TimesheetComplete") + "<br>"); //$NON-NLS-1$ //$NON-NLS-2$
		}
		if (indicators.isTimesheetClosed() && project.isTimesheetClosed()) {
			setLook(timesheetClosed,isSelected,hasFocus);
			label.add(timesheetClosed);
			text.append(Messages.getString("TaskIndicatorsComponent.TimesheetClosed")+ "<br>"); //$NON-NLS-1$ //$NON-NLS-2$

		}
		if (indicators instanceof Task && indicators.isParentWithAssignments()) {
