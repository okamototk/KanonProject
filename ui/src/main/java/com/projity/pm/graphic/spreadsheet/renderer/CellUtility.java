...
public class CellUtility {
	public static final JLabel GRAY = new JLabel();
	static {
		GRAY.setBackground(Colors.LIGHT_GRAY);
	}
...
//		if (node.isVoid()){
//			if (node.getLevel()==1) component.setBackground(Color.RED);
//			else if (node.getLevel()==2) component.setBackground(Color.ORANGE);
//			else component.setBackground(Color.GREEN);
//		}
...
//				component.setForeground(Color.GRAY);
	...
	public static boolean isHidden(JTable table, int row, int column) {
		...
		if (field == null ||  rowObject == null)
			return false;
		return field.isHidden(rowObject,null);
	}
