...
import com.projity.dialog.ResourceSubstitutionDialog;
...
public class DocumentFrame extends NamedFrame implements
	...
	public DocumentFrame(GraphicManager parentFrame, final Project project,String id) {
		...
		filterToolBarManager = (Environment.isNewLook()&&!Environment.isNoPodServer()) ? FilterToolBarManager.create(menuManager) : graphicManager.getFilterToolBarManager();
...
//		registerToolBarActions();
	...
	boolean doBaselineDialog(boolean save) {
		finishAnyOperations();
		getGraphicManager().lockProject(); // pre-lock project for timeout issue
		BaselineDialog baselineDialog = BaselineDialog.getInstance(getGraphicManager(),
				null, save,hasAtLeastOneTaskSelected());
		baselineDialog.setShowEnabled(!GraphicManager.getInstance().isProjectLockedByOther());
...
//		GroupDefinitionDialogBox.getInstance(getGraphicManager().getFrame(), null).doModal();
	...
	public void doCompleteAndClose() {
		finishAnyOperations();
		ScheduleUtil.completeAndClose(NodeList.filterNodeList(getSelectedNodes(true),false),getProject());
	}
...
//			parentFrame.setTitle(Messages.getString("Text.ApplicationTitle") + " - " + project.getName());
	...
	public void doSubstituteResource() {
		finishAnyOperations();
		final ResourceSubstitutionDialog dlg = ResourceSubstitutionDialog.getInstance(
				getGraphicManager().getFrame(), null,hasAtLeastOneTaskSelected(),getProject());
		if (dlg.doModal()) {
			Command command = new Command(Messages.getString("ResourceSubstitutionDialogBox.title"), getProject()) {
				public void execute(Object arg0) {
					long startingTime =dlg.getForm().getRescheduleDate();
					AssignmentService.getInstance().replaceResourceOnTask((NormalTask)arg0, dlg.getForm().getFromResource(), dlg.getForm().getToResource(), dlg.getForm().isIgnoreInProgress(),startingTime);
				}};
			int transactionId = project.fireMultipleTransaction(0,true);
			project.beginUndoUpdate();
			forTasksDo( command ,dlg.getForm().getEntireProject().booleanValue());
