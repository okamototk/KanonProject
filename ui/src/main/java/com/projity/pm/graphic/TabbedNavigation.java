...
public class TabbedNavigation implements MenuActionConstants, Serializable {
	...
	public JComponent createContentPanel(MenuManager menuManager,JToolBar toolbar, int group, int tabPlacement, boolean addFilterButtons) {
		...
		for (int i=0; i < toolbar.getComponentCount(); i++) {
			...
			for (String removed : Configuration.getInstance().getRemovedMenu()) {
				try {
					if (action == menuManager.getActionFromId(removed))
						removedItem = true;
				} catch (Exception e) {
					// benign
				}
			...
			if (action == menuManager.getActionFromId(ACTION_RBS))
				this.rbsTabCount  = tabCount;
