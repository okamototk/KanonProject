...
import com.projity.dialog.FeatureNotAvailableDialog;
...
//		I have submitted a bug report: https://substance.dev.java.net/issues/show_bug.cgi?id=155 with a proposed fix
		...
		((DefaultFrameManager)frameManager).cleanUp(false);
		currentFrame = null;
	...
	public static void cleanUpStatic(){
		//cleanUp static
		lastGraphicManager = null;
		graphicManagers=new LinkedList();
		lafManager=null;
		badLAF = false;
		lastWorkspace = null;
...
//			}
	...
	public void addHandlers() {
		...
		actionsMap.addHandler(ACTION_REVERT, new RevertAction());
		actionsMap.addHandler(ACTION_CANCEL_CHANGES, new CancelChangesAction());
		actionsMap.addHandler(ACTION_PUBLISH, new PublishAction());
		actionsMap.addHandler(ACTION_RELOAD, new ReloadAction());
		actionsMap.addHandler(ACTION_PULL_ACTUALS, new PullActualsAction());
		...
		actionsMap.addHandler(ACTION_SUBSTITUTE_RESOURCE, new SubstituteResourceAction());
...
//		actionsMap.addHandler(ACTION_UNDO_STACK, new UndoStackAction());
	...
	public class NewProjectAction extends MenuActionsMap.GlobalMenuAction {
		...
		public boolean allowed(boolean enable){
			DocumentFrame dframe = getCurrentFrame();
			...
			if (Environment.getStandAlone())
				...
				if (isDocumentActive()) {
			    	...
			    	if (!session.isSavable(project)) {
						if (JOptionPane.YES_OPTION == Alert.confirmYesNo(Messages.getString("Text.projectGotUnlocked"), JOptionPane.YES_OPTION))
...
//			return true;//!project.isLocal()&&!project.isMaster();
	...
	public class PublishAction extends SaveProjectAction {
		private static final long serialVersionUID = 1L;
		protected boolean isPublish() {
			return true;
		}
...
//		}
	...
	public class ReloadAction extends MenuActionsMap.DocumentMenuAction {
		private static final long serialVersionUID = 1L;
		public void actionPerformed(ActionEvent arg0) {
			setMeAsLastGraphicManager();
			if (isDocumentActive()) {
	...
	public class PullActualsAction extends MenuActionsMap.DocumentMenuAction {
		private static final long serialVersionUID = 1L;
		public void actionPerformed(ActionEvent arg0) {
			setMeAsLastGraphicManager();
			if (isDocumentActive()) {
...
//		    	}
		...
		public boolean allowed(boolean enable) {
			if (getCurrentFrame() == null)
				return false;
			Project project=getProject();
			return project != null && !project.isMspAssociated() && !project.isCheckedOutByOther() && project.isTimesheetAssociated();
		}
	}
	public class SubstituteResourceAction extends MenuActionsMap.DocumentMenuAction {
		private static final long serialVersionUID = 1L;
		public void actionPerformed(ActionEvent arg0) {
			setMeAsLastGraphicManager();
			if (isDocumentActive()) {
	...
	public class CancelChangesAction extends MenuActionsMap.DocumentMenuAction {
		private static final long serialVersionUID = 1L;
		public void actionPerformed(ActionEvent arg0) {
			setMeAsLastGraphicManager();
			if (isDocumentActive()) {
...
//			System.out.println("Cancel changes allowed canRevert " + canRevert + " needs saving " + project.needsSaving());
	...
	public class RevertAction extends MenuActionsMap.DocumentMenuAction {
		private static final long serialVersionUID = 1L;
		public void actionPerformed(ActionEvent arg0) {
			setMeAsLastGraphicManager();
 ...
 */
		public boolean allowed(boolean enable) {
			...
			if (project.isLocked() && isDocumentWritable()) //user has the lock AND plan has been modified (different from published server copy)
				return true;
			if (project.getCheckedOutUser() != null) {// if somebody has it
...
//	}
	...
	public class SaveBaselineAction extends MenuActionsMap.DocumentMenuAction {
		...
		public boolean allowed(boolean enable) {
			if (enable==false) return true;
			return isDocumentWritable() && !getCurrentProject().isCheckedOutByOther() && getCurrentProject().getProjectPermission().isWrite() && getCurrentProject().getProjectPermission().isSaveBaseline();
		}
	}
...
////		}
	...
	public synchronized void setEnabledDocumentMenuActions(boolean enable) {
		if (getProject() == null)
			return;
       ...
       actionsMap.setEnabledDocumentMenuActions(enable);
        if (getCurrentFrame() != null) {
        	...
        	((DefaultFrameManager)getFrameManager()).setLockStatus(getProject().getLockStatus(),getProject().getPublishInfoHtml(), getProject().getCheckedOutUser());
...
protected boolean loadLocalDocument(String fileName,boolean merge, boolean importCalendars){ //uses server to merge
	addHistory("loadLocalDocument",new Object[]{fileName,merge});
		...
		if (fileName==null) {
			...
			if (merge){
				ResourceMappingForm form=new ResourceMappingForm(){
					public boolean execute(){
						if (getImportedResources() == null || getImportedResources().size() == 0) // don't show dialog if no resources were imported
							return true;
			...
			if (fileName.endsWith(".pod")){ //$NON-NLS-1$
				...
				opt.setSkipCalendars(!importCalendars);
			}
...
//		};
	...
	public void openLocalProject(boolean importCalendars){
...
//			taskType && (!notVoid || currentImpl == null || ((Task)currentImpl).getOwningProject() == null || ((Task)currentImpl).getOwningProject() == project);
	...
	public class ReinitializeAction extends MenuActionsMap.GlobalMenuAction {
		private static final long serialVersionUID = 1L;
		public void actionPerformed(ActionEvent arg0) {
			setMeAsLastGraphicManager();
	...
	public class UndoStackAction extends MenuActionsMap.GlobalMenuAction {
		private static final long serialVersionUID = 1L;
		public void actionPerformed(ActionEvent arg0) {
			setMeAsLastGraphicManager();
...
//        container.invalidate();
	...
	public void doInsertTask(){
		actionsMap.getActionFromMenuId(GraphicManager.ACTION_INSERT_TASK).actionPerformed(null);
	}
    ...
    public void openFile(String fileName){
    	...
    	if (fileName!=null&&initialized) loadLocalDocument(fileName,!Environment.getStandAlone(), true);
    }
