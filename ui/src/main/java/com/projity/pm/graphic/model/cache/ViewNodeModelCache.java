...
//	}
	...
	public void deleteNodes(List nodes){
		...
		final boolean[] protectedFound=new boolean[]{false};
		for (Iterator i=nodes.iterator();i.hasNext()&&!protectedFound[0];){
			Node parent=(Node)i.next();
			getModel().getHierarchy().visitAll(parent,new Closure(){
				public void execute(Object arg){
					if (protectedFound[0]) return; //iterators would be better here
					Node node=(Node)arg;
					if (node.isProtected()){
