...
public class MenuActionsMap {
	Log log = LogFactory.getLog(MenuActionsMap.class);
	...
	public void addHandler(String menuId, AbstractAction action) {
		String listener = menuManager.getActionStringFromId(menuId);
		if(listener!=null){
			listeners.put(listener,action);
	        menuIdActionMap.put(menuId, action);
		} else {
			log.error("register action fail: '" + action+"' to menu '"+menuId+')');
		}
