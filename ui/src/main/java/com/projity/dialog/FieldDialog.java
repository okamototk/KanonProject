...
public abstract class FieldDialog extends AbstractDialog  implements ObjectEvent.Listener,ScheduleEventListener,SelectionNodeListener, DocumentSelectedEvent.Listener {
	...
	protected FieldComponentMap createMap() {
		...
		if (multipleObjects)
			...
			map = new FieldComponentMap(node, nodeModel);
		else
	...
	public void setObject(Object object) {
		...
		if (object != null && object instanceof BelongsToDocument) {
			...
			if (document instanceof Project){
				...
				nodeModel=project.getTaskOutline();
				node=nodeModel.search(object); //dialogs are base on object not node, need to search
				project.addScheduleListener(this);
			}else if (document instanceof ResourcePool){
				ResourcePool resourcePool=(ResourcePool)document;
				nodeModel=resourcePool.getResourceOutline();
				node=nodeModel.search(object);
			}else{
				node=null;
				nodeModel=null;
			}
