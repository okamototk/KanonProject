...
//System.out.println("new value " + newValue + " " + (newValue != null ?newValue.getClass():""));
		...
		try {
			...
			if (field.hasOptions())  {
				...
				Object key = ((JComboBox)component).getSelectedItem();
				field.setValue(objectRef,this,key,context,options);
			} else {
				if (field.isDate()) {
				...
				if (newValue != value){
					...
					if (field.isMoney()){
						...
						field.setText(objectRef,""+newValue,context,options);
					}
					else
						field.setValue(objectRef,source,newValue,context,options);
				}
			...
			if (options.getChange()!=null){
				...
				if (undoableEditSupport!=null){
					...
					undoableEditSupport.postEdit(new FieldEdit(field,objectRef,options.getChange().getNewValue(),options.getChange().getOldValue(),this,context,undoOptions));
				}
			}
			...
			if (field.hasOptions()) {
				Object oldValue=field.getValue(objectRef, context);
				((JComboBox)component).setSelectedItem(oldValue);
