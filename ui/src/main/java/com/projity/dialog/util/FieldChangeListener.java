 ...
 */
public class FieldChangeListener implements ItemListener,ChangeListener,ActionListener {
		...
		if (source instanceof JCheckBox){
			...
			try {
				if (objectRef.getNode()!=null&&objectRef.getNodeModel() instanceof NodeModel) //use nodeModel with undo
					((NodeModel)objectRef.getNodeModel()).setFieldValue(field, objectRef.getNode(), source, value, context,NodeModel.NORMAL);
				else{
					...
					field.setValue(objectRef,source,value,context,options);
					if (undoableEditSupport==null) Alert.warn(Messages.getString("Text.CannotUndoAction"));
					if (undoableEditSupport!=null&&options.getChange()!=null){
						...
						undoableEditSupport.postEdit(new FieldEdit(field,obj,options.getChange().getNewValue(),options.getChange().getOldValue(),source,context,undoOptions));
					}
				}
