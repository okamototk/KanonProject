...
public class DefaultNodeModel implements NodeModel {
	...
	public void paste(Node parent,List nodes,int position,int actionType){
		...
		if (summary!=null&&(parent==null||parent.isRoot())) parent=summary;
	...
	public void move(Node parent,List nodes,int position,int actionType){
    	...
    	int transactionId=DataFactoryUndoController.beginTransaction(this, actionType);
    	...
    	DataFactoryUndoController.endTransaction(this, actionType,transactionId);
	...
	public void remove(List nodes,int actionType,boolean filterAssignments,boolean removeDependencies){
		...
		try {
			Node summary=getSummaryNode();
			if (summary!=null&&nodes.contains(summary)){
				ArrayList nodes2=new ArrayList(nodes.size()-1);
				for (Object o:nodes) if (o!=summary) nodes2.add(o);
				nodes=nodes2;
			}
...
//		return parentNodes;
	...
	public void setFieldValue(Field field, Node node, Object eventSource, Object value, FieldContext context,int actionType) throws FieldParseException {
		setFieldValue(field, node, eventSource, value, context, null, actionType);
	}
	/* (non-Javadoc)
	 ...
	 */
	public void setFieldValue(Field field, Node node, Object eventSource, Object value, FieldContext context,FieldSetOptions options,int actionType) throws FieldParseException {
...
//			context.setUserObject(FieldContext.getNoUpdateInstance());
		...
		if (isUndo(actionType)){
			if (options==null) options=new FieldSetOptions();
			options.setRawUndo(true);
		}
		field.setValue(node, this,eventSource, value, context,options);
...
//		if (isEvent(actionType)) hierarchy.fireUpdate(new Node[]{node});
		...
		//Undo
		if (options!=null&&options.getChange()!=null){
			FieldSetOptions undoOptions=new FieldSetOptions();
			undoOptions.setRawProperties(true);
			ModelFieldEdit edit=new ModelFieldEdit(this,field,node,eventSource,options.getChange().getNewValue(),options.getChange().getOldValue(),context,undoOptions);
			postEdit(edit);
		}
	}
...
//		}
	...
	public Node replaceImplAndSetFieldValue(Node node, LinkedList previous, Object newImpl, Field field, Object eventSource, Object value,FieldContext context,int actionType) throws FieldParseException {
		...
		if (previous!=null){
			...
			Node parent=sibling==getSummaryNode()?sibling:(Node)sibling.getParent();
...
//	}
	...
	public Node getSummaryNode() {
		return hierarchy.getSummaryNode();
	}
	public Node getVirtualroot() {
		return hierarchy.getVirtualroot();
	}
	public boolean isSummaryNodeEnabled() {
		return hierarchy.isSummaryNodeEnabled();
	}
	public void setSummaryNodeEnabled(boolean summayTaskEnabled) {
		hierarchy.setSummaryNodeEnabled(summayTaskEnabled);
	}
