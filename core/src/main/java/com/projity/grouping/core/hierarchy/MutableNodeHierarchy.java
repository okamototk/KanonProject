...
//    }
    public void indent(List nodes,int deltaLevel, NodeModel model,int actionType){
    	if (deltaLevel<0){
			...
			if (summaryNode!=null){
				for (Iterator i=nodes.iterator();i.hasNext();){
					Node node=(Node)i.next();
					if (node.getParent()==summaryNode) return; //only works for -1
				}
			}
    	}
    	...
    	int transactionId=DataFactoryUndoController.beginTransaction(model, actionType);
		...
		DataFactoryUndoController.endTransaction(model, actionType, transactionId);
}
    ...
    private List internalIndent(List nodes,int deltaLevel,int actionType){
    	log.debug("(deltaLevel,actonType)=("+deltaLevel+","+actionType+")");
...
// Below is TreeModel implementation
	...
	public void checkEndVoidNodes(boolean subproject,int actionType){
		...
		if (!subproject) checkSubprojectEndVoidNodes(virtualRoot,inserted);
	...
	protected boolean summaryNodeEnabled;
	protected Node summaryNode;

   public boolean isSummaryNodeEnabled() {
		return summaryNodeEnabled;
	}

	public void setSummaryNodeEnabled(boolean summayTaskEnabled) {
		if (this.summaryNodeEnabled != summayTaskEnabled){
			this.summaryNodeEnabled = summayTaskEnabled;
			summaryNode=null;
		}
	}

	public Node getSummaryNode(){
		if (!summaryNodeEnabled) return null;
		if (summaryNode==null){
	...
	public Node getVirtualroot(){
		...
		if (sum==null) return (Node)getRoot();
		else return sum;
	}
