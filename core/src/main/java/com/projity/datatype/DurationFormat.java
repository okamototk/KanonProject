...
public class DurationFormat extends Format {
	...
	private static int getTypeUnit(String typeName){
		for (int i=0;i<TYPE_COUNT;i++){
			if (types[i].equals(typeName)) return i;
		}
		return -1;
	}
	public static String formatTypeUnit(String typeName) {
		DurationFormat.getInstance();
		int type=getTypeUnit(typeName);
		if (type==-1) return null;
		return formatTypeUnit(type,true,false,false,false,0);
	}
