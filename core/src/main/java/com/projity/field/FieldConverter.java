...
//	 make a converter for int
	private static class IntegerConverter implements Converter {
		Converter baseConverter = new org.apache.commons.beanutils.converters.IntegerConverter();
		public Object convert(Class type, Object value) throws ConversionException {
			if (value == null)
				return null;
			if (value != null) {
				if (value instanceof Date) {
					...
					return new Long(((Duration)value).getEncodedMillis());
				} else if (value instanceof String && type.equals(int.class)) {
	...
	private static class HyperlinkConverter implements Converter {
