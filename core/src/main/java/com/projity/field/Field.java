...
public class Field implements SummaryNames, Cloneable, Comparable, Finder, Comparator {
	...
	public final String convertIdToString(Object id) {
		...
		return (String) select.getKey(id,null);
	}
...
//			log.warn("unknown summary type: " + summaryType + " for field " + getName());
	...
	public Object[] getOptions(Object object) {
		if (select == null)
			return null;
		Object[] options=select.getKeyArray(object);
		if (filter==null)
			return options;
		else return filter.getOptions(options, select.getValueList(object), object);
	}
 ...
 */
	...
	public String toExternalText(Object value,Object obj) {
		if (hasOptions())
			return convertValueToStringUsingOptions(value,obj);
		else
	...
	public final String getText(Object object, FieldContext context) {
		...
		try {
			...
			if (hasOptions()) {
				return convertValueToStringUsingOptions(value,object);
			}
	...
	public String convertValueToStringUsingOptions(Object value, Object obj) {
		String result = (String) select.getKey(value,obj);
		if (result != null)
			return result;
...
//		else
	...
	private boolean nodeHasNonSummarizedValue(Node node, WalkersNodeModel nodeModel) {
		...
		if (node.getImpl() instanceof BelongsToDocument) { // for projects which don't roll up
			Document d = ((BelongsToDocument)node.getImpl()).getDocument();
			if (d.isDontSummarizeFields())
				return true;
		}
...
//		}z
		...
		if ("Field.duration".equals(id) && (object instanceof GroupNodeImpl)) {
			...
			if (nodeHasNonSummarizedValue(node, nodeModel)) {// if no summary
				...
				if (hasOptions()) {
					result = convertValueToStringUsingOptions(result,node.getImpl());
				}
			} else {
...
//		}
	...
	private static Object getSummarizedValueForField(Field field, Node node, WalkersNodeModel nodeModel, FieldContext context) {
		...
		Object key=null;
		Object result=null;
		walkingVisitor.setUseKey(false);//field.isHasOptions());
...
//		else
		...
		if (context != null && context.isHideNullValues() && walkingVisitor.getClosure() instanceof SummaryVisitor) { // special case for parents that have children all without value
			if (!((SummaryVisitor)walkingVisitor.getClosure()).isHasValue())
				return null;
		}
		if (field.isMoney()){
			result = Money.getInstance((Double)result);
		} else if (result instanceof Double) { // convert to proper display type
			result = ClassUtils.doubleToObject((Double) result, field.getDisplayType());
		}
		if (/*(object instanceof GroupNodeImpl) &&*/ field.hasOptions()) { // TODO should apply to summaries other than group
			result = field.convertValueToStringUsingOptions(result,object);
		}
//		System.out.println(node.getImpl() + " " + field + " value " + result);
		return result;
	}
	...
	public Object getValue(Object object) {
 	...
 	public Object getValue(Object object, FieldContext context) {
		...
		if (isMap()) {
			...
			if (result==null) return null; //Need to return null instead of false to detect a value is missing in the map. It's used by setInternalValue: if (value != null && value.equals(getValue(object, context)))
		}
...
//					result = Long.valueOf(Duration.millis(((Long)result).longValue()));
		...
		if (hideZeroValues && isZero(result))
			return null;
		else if (context !=null && context.isHideNullValues() && ClassUtils.isNull(result))
			return null;
...
//			return Duration.millis(((Duration)value).getEncodedMillis()) == 0L;
	...
	public Object setTextValue(Object object, String textValue, FieldContext context) throws FieldParseException {
		Format f = getFormat(object);
		Object value = null;
		if (f != null) {
			try {
	...
	public final void setText(Object object, String textValue, FieldContext context) throws FieldParseException {
		setText(object, textValue, context, null);
	}
	public final void setText(Object object, String textValue, FieldContext context, FieldSetOptions options) throws FieldParseException {
		if (context == null)
			context = specialFieldContext;
		...
		if (value == textValue) {
			value = setTextValue(object,textValue,context);
		}
		setInternalValueAndUpdate(object, this, value, context,options);
	}
	...
	public final void setText(Node node, WalkersNodeModel nodeModel, String textValue, FieldContext context) throws FieldParseException {
		setText(node, nodeModel, textValue, context,null);
	}
	public final void setText(Node node, WalkersNodeModel nodeModel, String textValue, FieldContext context, FieldSetOptions options) throws FieldParseException {
		if (context == null)
			context = specialFieldContext;
		if (!FieldContext.isForceValue(context) && isReadOnly(node, nodeModel, context)) { // don't allow setting of
													// read only fields
		// log.warn("Tried to set text of read only field" + getId());
			return;
		}
		...
		setValue(node, nodeModel,this, value, context,options);
	}

	public final void setText(ObjectRef objectRef, String textValue, FieldContext context) throws FieldParseException {
		setText(objectRef, textValue, context,null);
	}
	public final void setText(ObjectRef objectRef, String textValue, FieldContext context, FieldSetOptions options) throws FieldParseException {
		if (context == null)
			context = specialFieldContext;
		if (objectRef.getCollection() != null) {
			Iterator i = objectRef.getCollection().iterator();
			while (i.hasNext()) {
				setText(i.next(), textValue, context,options);
			}
		} else if (objectRef.getNode() != null)
			setText(objectRef.getNode(), objectRef.getNodeModel(), textValue, context,options);
		else
			setText(objectRef.getObject(), textValue, context,options);
	}
	 ...
	 */
	public void setValue(Node node, WalkersNodeModel nodeModel, Object source, Object value, FieldContext context, FieldSetOptions options) throws FieldParseException {
		if (context == null)
			context = specialFieldContext;
		Object object = node.getImpl();
		setValue(object, source, value, context,options);


		if (Environment.isNoPodServer()&&object instanceof DataObject){ //need to set ancestors dirty for rollup. The exposed fields have to be sent in this case
			if (context == null || !context.isNoDirty()){

				for (Node parent=nodeModel.getParent(node);parent!=null&&!parent.isRoot();parent=nodeModel.getParent(parent))
	...
	public void setValue(Node node, WalkersNodeModel nodeModel, Object source, Object value, FieldContext context) throws FieldParseException {
		setValue(node,nodeModel,source,value,context,null);
	}

	public void setValue(Object object, Object source, Object value, FieldContext context, FieldSetOptions options) throws FieldParseException {
		if (context == null)
			context = specialFieldContext;
...
//		if (value==null&&isBoolean()) value=false; //setting a boolean as null fails, it's possible with the undo of boolean field initialized at null
		if (options==null||!options.isRawProperties()){
			if (hasOptions()) {
				if (value instanceof String)
					value = preprocessText(object, (String) value, context);
			} else {
	...
	//			if (isBoolean()&&value==null) value=false;
		...
		setInternalValueAndUpdate(object, source, value, context,options);
//		if (context!=null&&context.getUndo() != null) {
...
//		}
	...
	public void setValue(Object object, Object source, Object value, FieldContext context) throws FieldParseException {
		setValue(object,source,value,context,null);
	}

	private void setInternalValueAndUpdate(Object object, Object source, Object value, FieldContext context, FieldSetOptions options) throws FieldParseException {
		if (context == null)
			context = specialFieldContext;
		if (setInternalValue(object, value, context,options)) { // if succeeded in setting value
	...
	public void setValue(ObjectRef objectRef, Object source, Object value, FieldContext context) throws FieldParseException {
		setValue(objectRef, source, value, context, null);
	}
	public void setValue(ObjectRef objectRef, Object source, Object value, FieldContext context, FieldSetOptions options) throws FieldParseException {
		if (context == null)
			context = specialFieldContext;
		if (objectRef.getCollection() != null) {
			Iterator i = objectRef.getCollection().iterator();
			while (i.hasNext()) {
				setValue(i.next(), source, value, context,options);
			}
		} else if (objectRef.getNode() != null)
			setValue(objectRef.getNode(), objectRef.getNodeModel(), source, value, context,options);
		else
			setValue(objectRef.getObject(), source, value, context,options);
	}
	...
	public boolean setInternalValue(Object object, Object value, FieldContext context, FieldSetOptions options) throws FieldParseException {
		if (context == null)
			context = specialFieldContext;
		if (!FieldContext.isForceValue(context) && isReadOnly(object, context)) { // don't allow setting of read only
											// fields
		...
		if (options==null||!options.isRawProperties()){
			...
			if (hasExternalType()) { // does the second pass, converting from
										// say, Date to long
				Class type = isMap() ? externalType : internalType;
		...
		try {
			if (isMap()) {
				...
				map.put(id, value);
				if (options!=null&&options.isRawUndo()){
					options.setChange(new ObjectChange(value,oldValue));
				}
				...
				if (options!=null&&options.isRawUndo()){
					if (getHasNoContext){
							oldValue = methodGet.invoke(object, (isIndexed() ? new Object[] { new Integer(index) } : new Object[] {}));
					}else{
						oldValue = methodGet.invoke(object, (isIndexed() ? new Object[] { new Integer(index), context }
						: new Object[] { context }));
					}
				}
				if (setHasNoContext) {
				...
				if (options!=null&&options.isRawUndo()){
					Object newValue;
					if (getHasNoContext){
						newValue = methodGet.invoke(object, (isIndexed() ? new Object[] { new Integer(index) } : new Object[] {}));
					}else{
						newValue = methodGet.invoke(object, (isIndexed() ? new Object[] { new Integer(index), context }
						: new Object[] { context }));
					}
					options.setChange(new ObjectChange(newValue,oldValue));
				}
	...
	public boolean isReadOnly(Object object, FieldContext context) {
		...
		if ((object instanceof DelegatesFields) && ((DelegatesFields)object).delegates(this))
			return true;
		Boolean value = (Boolean) invokeContextMethod(methodReadOnly, object, context, readOnlyHasNoContext,true);
		if (value != null)
	...
	public boolean isHidden(Object object, FieldContext context) {
		...
		if (methodHide == null)
			return false;
		Boolean value = (Boolean) invokeContextMethod(methodHide, object, context, hideHasNoContext, false);
		if (value != null)
			return value.booleanValue();
		return true;
...
//		return false;
	...
	public String mapValueToString(Object value) {
		if (select == null)
			return null;
		return (String) select.getKey(value,null);
	}
...
//			methodObjectReadOnly = MethodUtils.getAccessibleMethod(clazz, "isReadOnly", getterParams);
	...
	private final Object invokeContextMethod(Method method, Object object, FieldContext context, boolean noContext, boolean dumpException) {
		if (context == null)
			context = specialFieldContext;
		if (method == null)
			return null;
...
//		formula = FormulaFactory.addScripted("Field", formulaName, variableName, formulaText);
	...
	public boolean isResourceNames() {
		return resourceNames;
	}

	public void setResourceNames(boolean resourceNames) {
...
//				System.out.println("____ref id = " + result);
	...
	public String getMetadataString() {
		...
		+ "\t" + typeName()
		+ "\t" + isReadOnly()
		+ "\t" + (getSummaryType() == null ? "-" : getSummaryType())
		+ "\t";
		if (hasDynamicSelect()) {
...
//System.out.println("Field "+ this + " group " + getGroupSummary() +  " sum " + getSummary());
	...
	public static String longName(String shortFieldName) {
...
//			return shortFieldName;
	...
	public static String longCustomName(String shortFieldName) {
		return "CustomField."+shortFieldName;
	}
	public boolean isNotChoosable() {
		return notChoosable;
	}

	public void setNotChoosable(boolean notChoosable) {
	...
	public void setSelectPopulateMethod(String populateMethod) {
		StaticSelect select = new StaticSelect();
		select.setPopulateMethod(populateMethod);
		setSelect(select);
	}

	public boolean isOnlyChangeOnPublish() {
		return onlyChangeOnPublish;
	}

	public void setOnlyChangeOnPublish(boolean onlyChangeOnPublish) {
		this.onlyChangeOnPublish = onlyChangeOnPublish;
	}
	...
	public void setListElementsField(Field listElementsField) {
		this.listElementsField = listElementsField;
	}
