...
public abstract class AlgorithmFieldUpdater extends Thread {
	private static Log log = LogFactory.getLog(AlgorithmFieldUpdater.class);
	Object eventSource;
	Document document;
 ...
 */	public void fireOutputEvents(Object algo, Object object) {
	...
	protected void addInputField(String fieldId) {
		...
		if (field == null) {
			log.error("Could not add input field (probably not yet implemented)" + fieldId);
		} else {
	...
	protected void addOutputField(String fieldId) {
		Field field = Configuration.getFieldFromId(fieldId);
		if (field == null) {
			log.error("Could not add output field (probably not yet implemented)" + fieldId);
		} else {
