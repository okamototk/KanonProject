...
public class Dictionary implements ProvidesDigesterEvents {
	...
	public static synchronized Dictionary getInstance() {
		if (instance == null) {
			...
			String additionalConfiguration=Messages.getMetaString("AdditionalViewConfiguration");
			ClassUtils.invokeVoidStaticMethodFromFullName(additionalConfiguration);

			if (Environment.isClientSide() /*&& !Environment.isTesting()*/) // this screws up on server and is not needed anyway
...
//this is actually normal if overriding with another xml file				ConfigurationReader.log.warn("named item " + namedItem + " already in category " + category);
	...
	public static void cleanUp() {
		instance = null;
		namedItemComparator = new NamedItemComparator();
	}
