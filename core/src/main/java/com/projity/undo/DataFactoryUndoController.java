...
public class DataFactoryUndoController extends UndoController {
	...
	private static Predicate hook = null;
	...
	public void undo() {
		if (hook == null || hook.evaluate(null)) {
			dataFactory.setGroupDirty(true);
			super.undo();
		}
	}
	...
	public static void setGlobalHook(Predicate h) {
		hook = h;
	}

	public static int beginTransaction(NodeModel model,int actionType){
    	boolean doTransaction = model.getDocument() != null;
    	int transactionId = 0;
     	DataFactoryUndoController undoController=null;
    	if (isUndo(actionType)){
    		undoController=model.getDataFactory()==null?null:model.getDataFactory().getUndoController();
    	}
	...
	public static void endTransaction(NodeModel model,int actionType,int transactionId){
		boolean doTransaction = model.getDocument() != null;
		DataFactoryUndoController undoController=null;
    	if (isUndo(actionType)){
    		undoController=model.getDataFactory()==null?null:model.getDataFactory().getUndoController();
