...
public class NormalTask extends Task implements Allocation, TaskSpecificFields,
		...
		HasTaskIndicators, HasExtraFields, DelegatesFields {
...
//		return calcActiveAssignmentDuration(getEffectiveWorkCalendar());
	...
	public void setDuration(long duration) {//,FieldContext context) {
		// JGao 12/8/2009 Added the following logic for handling changing a fixed duration task which has 0 work (no assignment). This is due to the change I am also
		// making to fix the behavior of changing 0 work fixed duration task. Basically, the units of the default assignment is 0 for fixed duration task
		// which has 0 work. The change is to reset the units of the default assignment to 1.0 in order for the duration to be changed correctly. However,
		// this change is also used by the code path here. As a result, the work will also changed according to the new duration. Just set to 0 if it fits
		// the creteria of mentioned case.
		boolean resetAssignmentUnits = false;
		AssociationList assignments = this.getAssignments();

		if (assignments.size() == 1 && ((Assignment)assignments.getFirst()).isDefault() && getWork() == 0.0 && getRemainingUnits() == 0.0)
			resetAssignmentUnits = true;

		if (getActualDuration() > Duration.millis(duration)) { // dont allow reduction of actuals if actuals are protected
...
//		setDurationWithActualsHandling(duration,true);
//	}
	private void setDurationWithActualsHandling(long duration, boolean reduceActualsIfReducingDurationToLessThanActuals) {
		...
		if (reduceActualsIfReducingDurationToLessThanActuals) {
	...
	public void setActualDuration(long actualDuration) {
		...
		if (actualDuration > getDurationMillis()) {
			setDurationWithActualsHandling(actualDuration,false);
 ...
 ***********************************************************************************/
	...
	public void addAssignment(Assignment assignment) {
		...
		if (!assignment.isDefault()) {
			...
			if (defaultAssignment != null ) { //Remove any default assignment
				if (getProject().isActualsProtected()) {
	...
	public void setFixedCost(double fixedCost) {
		if (getProject().getCurrencyRateTable() != null) {
			double rate = getProject().getCurrencyRateTable().calculateWeightedRateOverPeriod(getStart(),getEnd(),getEffectiveWorkCalendar());
			fixedCost /= rate; // convert to native currency
		}
...
//			return 1.0;
	...
	public void setWork(long work, FieldContext context) {
		if (isActualsProtected() && work < getActualWork(context))
...
//		System.out.println("adjusting actual start to " + new java.util.Date(start));
	 ...
	 * @param stop
	 */
	private void setStop(long stop, boolean limitToEnd) {
		if (stop == getStop())
			return;
...
//		scheduleWindow.setStop(stop);
	...
	public double baselineCost(long start, long end) {
		if (getBaselineSnapshot() == null)
			return 0;
		return getBaselineSnapshot().cost(start, end) + getBaselineSnapshot().fixedCost(start, end);
	}
	...
	public double fixedCost(long start, long end) {
		...
		if (getProject().getCurrencyRateTable() != null) {
			CurrencyRate rate = (CurrencyRate) getProject().getCurrencyRateTable().findActive(getEnd()); // just use the end date - hard to do prorated
			fixed *= rate.getRate();
		}
	...
	protected double getBaselineFixedCost(int numBaseline, FieldContext fieldContext) {
		TaskSnapshot snapshot = (TaskSnapshot) getSnapshot(new Integer(
				numBaseline));
		if (snapshot == null)
			return 0.0D;
	...
	public double getBaselineCost(int numBaseline, FieldContext fieldContext) {
		...
		double cost =snapshot.cost(
				FieldContext.start(fieldContext), FieldContext
						.end(fieldContext));
		double fixedCost = getBaselineFixedCost(numBaseline,fieldContext);
...
//		}
	...
	public void setEnd(long end, FieldContext context) {
...
//	    initializeDates();
	...
	public Object clone(){
		NormalTask task=(NormalTask)super.clone();
//		task.barClosureInstance = new BarClosure();
	...
	public void setActualWork(long actualWork, FieldContext context) {
		...
		if (FieldContext.hasInterval(context)) {
			...
			if (workValue == 0L) {
				...
				if (workValue > getWork(context)) {
					setWork(actualWork);
					increased = true;
				}
				boolean allowDefault = !isWbsParent();
				long  date = ReverseQuery.getDateAtValue(WORK, this, workValue, allowDefault);
	...
	public void setRemainingWork(long remainingWork, FieldContext fieldContext) {
		...
		setWork(getWork(fieldContext) + (remainingWork - oldRemaining));
//
//		setActualWork(getWork(fieldContext) - Duration.millis(remainingWork), fieldContext);
	...
	public boolean isTestImportOk() {
		...
		if (!ok) {
			...
			System.out.println(getName() + " " + getId() + " not ok "

//					+ DurationFormat.format(getDurationMillis()) + "    -    " + DurationFormat.format(getCustomDuration(0)));
					+ "start " + (DateTime.dayFloor(getStart()) == DateTime.dayFloor(getCustomDate(0)) )
					+" \nend " + (DateTime.dayFloor(getEnd()) == DateTime.dayFloor(getCustomDate(1)))
					+" \nend prj " + DateTime.medString(DateTime.dayFloor(getEnd())) + " mar " + DateTime.medString(DateTime.dayFloor(getCustomDate(1)))
					+ " \ndur " +  (getDurationMillis() == Duration.millis(getCustomDuration(0)))
					+ " \noutline " + getOutlineString()  + " " + getCustomText(0) );
			System.out.println("start---end " + DateTime.medString(getStart()) + "  " +  DateTime.medString(getEnd()));
			System.out.println("dura proj --- mar " + DurationFormat.format(getDurationMillis()) + "  " +  DurationFormat.format(getCustomDuration(0)));
		}
//		else
//			dumpTest();
		return ok;
...
System.out.println("dura proj --- mar " + DurationFormat.format(getDurationMillis()) + "  " +  DurationFormat.format(getCustomDuration(0)));
	...
	public boolean isReadOnlyPercentComplete(FieldContext fieldContext) {
		return isActualsProtected();
	}
	...
	public boolean isProtected() {
		...
		for (Assignment assignment:(Collection<Assignment>)assignments){ //needed? done below and in hierachy anyway
	...
	public void setInvestmentMilestone(String investmentMilestone) throws FieldParseException {
		...
		getExtraFields().put(f.getAlternateId(), investmentMilestone);
	...
	public void setExternallyVisible(boolean externallyVisible) {
		Field f = Configuration.getFieldFromId("Field.externallyVisible");
		getExtraFields().put(f.getAlternateId(), externallyVisible);
	}
	public boolean isReadOnlyExternallyVisible(FieldContext fieldContext) { // readonly if milestone
		if (isCrossProject())
			return true;
...
//	}
	public boolean isReadOnlyCost(FieldContext fieldContext) {
		return (!Environment.isNoPodServer() || hasRealAssignments() || isReadOnlyFixedCost(fieldContext));
	}
	public void setCost(double cost, FieldContext context) {
		setFixedCost(cost);
	}
	...
	public void setTimesheetClosed(boolean timesheetClosed) {
		if (!hasRealAssignments())
			return;
		...
		while (i.hasNext()) {
			Assignment ass = ((Assignment)i.next());
			ass.setTimesheetClosed(timesheetClosed);
		}
	}
	public boolean isTimesheetComplete() {
		...
		if (isWbsParent()) {
			...
			while (j.hasNext()) {
				NormalTask t = (NormalTask)j.next();
	...
	public void setTimesheetComplete(boolean resourceComplete) {
		// for debugging
	}
	...
	public boolean fieldHideTimesheetClosed(FieldContext fieldContext) {
		return !getProject().isTimesheetClosed();
	}
	...
	public boolean isReadOnlyPercentWorkComplete(FieldContext context) {
		return isActualsProtected();
	}
	...
	public boolean isReadOnlyTimesheetClosed(FieldContext fieldContext) {
		return !getProject().isTimesheetAssociated() || (!isZeroDuration() && !isComplete());
	}
	...
	public void setCapEx(boolean value) {
		Field f = Configuration.getFieldFromId("Field.capEx");
		getExtraFields().put(f.getAlternateId(), value);
	}
