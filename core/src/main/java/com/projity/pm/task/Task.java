...
public abstract class Task implements HasKey, HasNotes, HasCalendar, HasDependencies, Schedule, ScheduleWindow,  Snapshottable, HasTimeDistributedData, HasPriority, CustomFields, BelongsToDocument, BelongsToHierarchy, DataObject, CanBeLeveled,UpdatesFromTimesheet,HasExpenseType,TaskLinkReference {
	...
	protected transient boolean baselineTouched[] = null;

	public static Field getStartField() {
		return Configuration.getFieldFromId("Field.start");
	}
	/**
	...
	public boolean isBaselineTouched(Object numBaseline) {
		if (baselineTouched == null)
			return false;
		int baseline = (Integer)numBaseline;
		return baselineTouched[baseline];
	}

	public void touchBaseline(Object numBaseline) {
		if (baselineTouched == null)
 ...
 */	public String getPredecessorOrder() {
 ...
 protected List persistedAssignments;
 public List getPersistedAssignments() {
	return persistedAssignments;
 }
 public void setPersistedAssignments(List persistedAssignments) {
	this.persistedAssignments = persistedAssignments;
 }
	...
	public void setRoot(boolean root){
		if (getUniqueId()!=DataObject.SUMMARY_UNIQUE_ID&&root)
...
//			hasKey.renumber(hasKey.isLocal()); //not tested, Shoudn't occur anyway
	...
	public void setMeAndAncestorsDirty(boolean dirty) {
		setDirty(dirty);
		Task t = getWbsParentTask();
		if (t != null)
			t.setMeAndAncestorsDirty(dirty);
	}
}
