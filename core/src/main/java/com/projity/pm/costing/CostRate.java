...
public class CostRate extends ValueObjectForInterval implements Cost {
	...
	@Override
	protected void setPrimaryValue(double value) {
		standardRate = new Rate(value,Environment.isNoPodServer() ? TimeUnit.HOURS : TimeUnit.DAYS);
	}
	...
	public void serializeCompact(ObjectOutputStream s,ValueObjectForIntervalTable parentTable) throws IOException {
		super.serializeCompact(s,parentTable);
		s.writeDouble(costPerUse);
	    standardRate.serialize(s);
	    overtimeRate.serialize(s);
	}

	public void deserializeCompact(ObjectInputStream s,ValueObjectForIntervalTable parentTable) throws IOException, ClassNotFoundException  {
		super.deserializeCompact(s,parentTable);
		costPerUse=s.readDouble();
	    standardRate=Rate.deserialize(s);
	    overtimeRate=Rate.deserialize(s);
	}

	protected ValueObjectForIntervalTable createValueObjectTable() {
		return new CostRateTable();
	}
	public String toString() {
		return "Start: " + DateTime.shortString(getStart())  + " End: " + DateTime.shortString(getEnd()) + " Value:" + getPrimaryValue()* WorkCalendar.MILLIS_IN_HOUR;
	}
