...
//		}
	...
	public void serializeCompact(ObjectOutputStream s) throws IOException {
	    ...
	    if (costRateTableArray==null){
		    ...
		    s.writeInt(costRateTableArray.length);
		    for (int i=0;i<costRateTableArray.length;i++){
		    	s.writeBoolean(costRateTableArray[i]!=null);
		    	if (costRateTableArray[i]!=null) costRateTableArray[i].serializeCompact(s);
		    }
	    }
	}
	...
	public static CostRateTables deserializeCompact(ObjectInputStream s) throws IOException, ClassNotFoundException  {
		CostRateTables t=new CostRateTables();
		t.names=(String[])s.readObject();
		int count=s.readInt();
		t.costRateTableArray=null;
		if (count>=0){
			t.costRateTableArray=new CostRateTable[count];
		    for (int i=0;i<count;i++){
		    	boolean hasTable=s.readBoolean();
		    	if (hasTable){
		    		CostRateTable ct=new CostRateTable();
		    		ct.deserializeCompact(s);
		    		t.costRateTableArray[i]=ct;
		    	}
		    }
		}
		return t;
