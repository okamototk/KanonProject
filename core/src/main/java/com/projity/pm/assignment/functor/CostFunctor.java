...
public class CostFunctor extends AssignmentFieldOvertimeFunctor {
	...
	double work = 0.0D;
	public static CostFunctor getInstance(Assignment assignment, WorkCalendar workCalendar, ContourBucketIntervalGenerator contourBucketIntervalGenerator, double overtimeUnits, com.projity.algorithm.CollectionIntervalGenerator costRateGenerator, CollectionIntervalGenerator currencyRateGenerator,long fixedCostDate, boolean proratedCost) {
		return new CostFunctor(assignment, workCalendar, contourBucketIntervalGenerator, overtimeUnits, costRateGenerator, currencyRateGenerator, fixedCostDate, proratedCost);
	}
	private CostFunctor(Assignment assignment, WorkCalendar workCalendar, ContourBucketIntervalGenerator contourBucketIntervalGenerator, double overtimeUnits, CollectionIntervalGenerator costRateGenerator, CollectionIntervalGenerator currencyRateGenerator, long fixedCostDate, boolean proratedCost) {
		...
		if (bucket != null) {
			...
			if (bucketUnits != 0.0) { // there are never values if there is no normal cost.
				...
				if (assignment.isTemporal()) { // for work resources or time based material
					...
					bucketOvertime *= Math.round((overtimeUnits * bucketDuration/WorkCalendar.MILLIS_IN_HOUR) * 100) / 100.0;
				}
				else{
					bucketRegular *= bucketUnits;
					bucketOvertime *= overtimeUnits;
				}
				...
				if (costPerUse != 0.0D) {
					...
					double bucketFixed = fraction * costPerUse * assignment.getUnits() * currencyRate;
