...
public class PersonalContour extends AbstractContour {
	...
	public static PersonalContour addDefaultBucket(AbstractContour from, long duration, boolean after, double units)
	{
		int oldLength = from.contourBuckets.length;
		PersonalContour newContour = getInstance(new AbstractContourBucket[oldLength+1]);

		System.arraycopy(from.contourBuckets, 0, newContour.contourBuckets, after ? 0 : 1, oldLength); // replace old with new and truncate to correct length
		newContour.contourBuckets[after ? oldLength : 0] = PersonalContourBucket.getInstance(duration, units);
		return newContour;
	}
 ...
 /**
	 ...
	 *
	 * JGao 8/13/2009 I changed the logic here. This no longer always preserves the number of buckets.
	 * If newDuration is longer than the sum of the duration of all buckets. It only extends the last bucket if it has the same
	 * units. Otherwise, a new bucket is created with that units.
	 */
	public AbstractContour adjustDuration(long newDuration, long actualDuration, double units) {
		...
		// extend last bucket to account for duration if same units, otherwise create a new bucket
		if (bucket.getUnits() == units)
			newContour.contourBuckets[contourBuckets.length-1] = bucket.adjustDuration(newDuration);
		else
			newContour = addDefaultBucket(this, newDuration, true, units);
