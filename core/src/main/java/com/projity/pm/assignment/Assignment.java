...
//		}
	...
	public void adjustWorkContourForEarlierTaskStart( long earlierStart, long lateStart ){
...
//		}
	...
	public void adjustRemainingWork(long remainingWork) {
...
//			System.out.println("after shift" + getWorkContour());
	...
	public void addWorkingTimeIfRequired(long date) {
		WorkCalendar cal = getEffectiveWorkCalendar();
	...
	private AssignmentFieldFunctor cost(SelectFrom clause, boolean all) {
		if (detail.getCostContour().isPersonal()) { // in the case where a cost contour has already been saved, use it
			...
			if (all || prorated) {
				...
				clause.from(costRate).from(workContour); // maybe add currency rate table after

				CurrencyRateTable currencyRateTable = getProject().getCurrencyRateTable();
				CollectionIntervalGenerator currencyRate = null;
				if (currencyRateTable != null && currencyRateTable.isActive()) {
					currencyRate = CollectionIntervalGenerator.getInstance(currencyRateTable.getList()); // go through project which will delegate to global value. Should realy be project related, not global
					clause.from(currencyRate);
				}
				// Note that the getStart() parameter implies cost per use is applied at start
				CostFunctor costF = CostFunctor.getInstance(this, getEffectiveWorkCalendar(), workContour, detail.calcOvertimeUnits(), costRate,currencyRate, getStart(),prorated);
				clause.select(costF);
				return costF;
	...
	public void setInterval(Object type, long start, long end, double value) {
		...
		if (type == ACTUAL_WORK || type == WORK || type == REMAINING_WORK) {
			...
			if (Environment.isImporting()) {
				...
				if (type != ACTUAL_WORK && Environment.isNoPodServer())
...
//System.out.println("baseline " + baseline + " " + new Date(start) + " " + new Date(end) + " " + value);
	 ...
	 */
	private void setWorkInterval(long start, long end, double value, boolean forceValue) {
		moveDelayToContour();
...
//			detail.recalculateDuration();
	...
	public int compareTo(Object o) {
		return getResourceName().compareTo(((Assignment)o).getResourceName());
	}
	...
	public boolean setEstimatedValues(long estimatedRemainingWork, long estimatedWork, long estimatedStart, long estimatedFinish, double estimatedPercentComplete) {
	...
	public void setEstimatedRemainingWork(long estimatedRemainingWork) {
		this.estimatedRemainingWork = estimatedRemainingWork;
	}
	...
	public void setEstimatedWork(long estimatedWork) {
		this.estimatedWork = estimatedWork;
	}
	...
	public void setEstimatedPercentComplete(double estimatedPercentComplete) {
		this.estimatedPercentComplete = estimatedPercentComplete;
	}
	...
	public void setCreatedFromTimesheet(boolean createdFromTimesheet) {
		this.createdFromTimesheet = createdFromTimesheet;
	}

	public void setRawDurationOfDetail(long duration) {
