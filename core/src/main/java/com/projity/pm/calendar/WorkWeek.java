...
//    		System.out.println("day num is " + dayNum);
	...
	public void serialiseCompact(ObjectOutputStream s) throws IOException{
		//s.writeLong(workingDuration); //calculated
		if (workDay==null){
			s.writeInt(-1);
		}else{
			s.writeInt(workDay.length);
			for (int i=0;i<workDay.length;i++){
				s.writeBoolean(workDay[i]!=null);
				if (workDay[i]!=null) workDay[i].serialiseCompact(s);
			}
		}
	}
	public static WorkWeek deserialiseCompact(ObjectInputStream s) throws IOException, ClassNotFoundException{
		//long duration=s.readLong();
		int count=s.readInt();
		WorkDay[] workDays=null;
		if (count>=0){
			workDays=new WorkDay[count];
			for (int i=0;i<count;i++){
				workDays[i]=s.readBoolean()?WorkDay.deserialiseCompact(s):null;
			}
		}
		return new WorkWeek(workDays);
	}
