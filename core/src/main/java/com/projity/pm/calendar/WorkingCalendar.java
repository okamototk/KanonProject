...
//			CalendarService.getInstance().add(this);
	...
	public WorkDay findExceptionDay(long date) {
		return differences.findExceptionDay(date);
	}
...
//			DebugUtils.dumpStack("0 date");
	...
	public static void setStandardInstance(WorkingCalendar standard) {
		standardInstance = standard;
		defaultInstance = getStandardBasedInstance();
		defaultInstance.setName(standard.getName());
	...
	public static void setDefaultInstance(WorkingCalendar wc) {
		defaultInstance = wc;
	}
...
//		}
	...
	public void serialiseCompact(ObjectOutputStream s) throws IOException{
		serializedName = getName();
		if (baseCalendar == CalendarService.getInstance().getStandardInstance()) // don't serialize default base. treat it as null
			baseCalendar = null;

		s.writeByte(version);
		s.writeByte(baseCalendar==null?0:(baseCalendar instanceof WorkingCalendar?1:2)); //bad, to compress
		if (baseCalendar!=null) baseCalendar.serialiseCompact(s);
		differences.serialiseCompact(s);
		s.writeInt(fixedId);
		s.writeObject(serializedName);
	...
	public void deserialiseCompact(ObjectInputStream s) throws IOException, ClassNotFoundException{
	    version=s.readByte();
	    byte baseCalendarType=s.readByte();
	    if(baseCalendarType>0){
	    	baseCalendar=baseCalendarType==1?WorkingCalendar.getInstance():new CalendarDefinition();
	    	baseCalendar.deserialiseCompact(s);
	    }else baseCalendar=null;
	    differences = new CalendarDefinition();
	    differences.deserialiseCompact(s);
	    fixedId=s.readInt();
	    serializedName=(String)s.readObject();
...
//		start = DateTime.dayFloor(start);
		addCalendarTime( start, DateTime.nextDay(start) );
