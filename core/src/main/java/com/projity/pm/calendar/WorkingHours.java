...
public class WorkingHours implements Cloneable, Serializable {
	...
	public void subtract(WorkRange otherRange) {
		ArrayList list = new ArrayList();
		long start;
		long end;
		int count = 0;
		for (WorkRange thisRange : workRange) {
			if (thisRange == null)
				continue;
			if (!thisRange.overlaps(otherRange)) {
				...
				try {
				start = thisRange.getStart();
				end  =  otherRange.getStart();
				if (start < end)
					list.add(new WorkRange(start,end));
				start  =  otherRange.getEnd();
		...
		workRange = new WorkRange[list.size()];
		list.toArray(workRange);
		initialize();
	}

	public void add(WorkRange addedRange) {
		ArrayList list = new ArrayList();
		for (WorkRange thisRange: workRange) {
			if (thisRange == null)
				continue;
			if (addedRange != null && thisRange.overlaps(addedRange)) { // if overlaps
				try {
					addedRange = new WorkRange(Math.min(addedRange.getStart(), thisRange.getStart()), // enlarge added range
							Math.max(addedRange.getEnd(),thisRange.getEnd()));
				} catch (WorkRangeException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			} else { // no overlap
				if (addedRange != null && addedRange.isBefore(thisRange)) { // add in added range since it is before
					list.add(addedRange);
					addedRange = null;
				}
				list.add(thisRange); // add in current range
			}
		}
		if (addedRange != null) // if added range left over at end, add it
			list.add(addedRange);
 ...
 */
	WorkingHours intersectWith(WorkingHours other) {
		...
		for(;;) {
			...
			// is this code ok? what if there are ranges left? -hk 2/2/09
...
/**
 *
 * @param startHour - can be fractional
 * @param hours - can be fractional
 * @return
 */
	public static WorkingHours createContiguousNHourDay(double startHour,double hours) {
		WorkingHours h = new WorkingHours();
		h.add(startHour, hours);
		return h;
	}

	public void add(double startHour,double hours) {
		add(WorkRange.createFromStartAndNumber(startHour, hours));
	}
	public void subtract(double startHour,double hours) {
		subtract(WorkRange.createFromStartAndNumber(startHour, hours));
	}
	...
	public boolean hasHours() {
		// JGao 9/26/2009 Removed the line for settting duration to 0. To me this is a mistake, as this function just check to see if there is any
		// work ragne that is not null, it SHOULD NOT change the duration of the working hours. This little incidental error caused me grief and two hours
		// of frustration and perplexed.
		for (int i =0; i <workRange.length; i++) {
			if (workRange[i] != null)
				return true;
		}
