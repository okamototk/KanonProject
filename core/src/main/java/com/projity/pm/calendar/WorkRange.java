...
//		cal.roll(GregorianCalendar.HOUR_OF_DAY,false);
	...
	public static WorkRange createFromStartAndNumber(double startHour,double hours) {
		if (startHour + hours > 24) // avoid wrap
			hours = 24 - startHour;
		Calendar cal = DateTime.calendarInstance();
		cal.setTimeInMillis(0);
		int hour = (int) Math.floor(startHour);
		double fractionalHours = startHour - hour;
		cal.set(Calendar.HOUR_OF_DAY,hour); // set the hour to avoid weird daylight issues if on the crossover day
		long start = cal.getTimeInMillis();
		start += fractionalHours * WorkCalendar.MILLIS_IN_HOUR;
		long end = (long) (start + WorkCalendar.MILLIS_IN_HOUR * hours);
		try {
