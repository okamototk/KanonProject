...
//	}
	...
	private void writeObject(ObjectOutputStream s) throws IOException {
	    ...
	    workCalendar=wc;
	    hasKey.serialize(s);
	    costRateTables.serializeCompact(s);
	    customFields.serialize(s);
	    s.writeInt(hasAssignments.getSchedulingType());
    	s.writeBoolean(hasAssignments.isEffortDriven());
	    availabilityTable.serializeCompact(s);
	    if (version>=4){
	    	s.writeBoolean(wc!=null);
	    	if (wc!=null) wc.serialiseCompact(s);
	    }
	}
	...
	private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException  {
	    ...
	    costRateTables=version>=3?CostRateTables.deserializeCompact(s):CostRateTables.deserialize(s);
	    try {
	    ...
	    if (version>=2){
	    	...
	    	if (version==2) availabilityTable=AvailabilityTable.deserialize(s);
	    	else{
	    		availabilityTable=new AvailabilityTable();
	    		availabilityTable.deserializeCompact(s);
	    	}
	    }else availabilityTable=new AvailabilityTable(null);
	    if (version>=4){
	    	if (s.readBoolean()){
	    		workCalendar=WorkingCalendar.getInstance(); //assume WorkingCalendar impl
	    		workCalendar.deserialiseCompact(s);
	    	}
	    }
	    version=DEFAULT_VERSION;
 	}

	public static void dumpSize(String name,Closure c){
        	try {
				ObjectOutputStream out=new ObjectOutputStream(new FileOutputStream("/Users/chretien/tmp/r."+name+"."+System.currentTimeMillis()));
				c.execute(out);
