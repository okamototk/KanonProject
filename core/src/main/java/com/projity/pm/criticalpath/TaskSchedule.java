...
//	}
	...
	public final void setStart(long start) {
		...
		if (start < this.start){
			NormalTask nt = (NormalTask)this.task;

			Assignment assignment;
			Iterator i = nt.getAssignments().iterator();
			while (i.hasNext()) {
				assignment = (Assignment)i.next();
				if (!assignment.isDefault() && assignment.getPercentComplete() > 0 && assignment.getTask().getProject().isActualsProtected())
					assignment.adjustWorkContourForEarlierTaskStart( start, this.start );
			}
...
//		System.out.println("Affects" + (oppositeEnd < newEnd) + " opposite " + new Date(oppositeEnd) + " new" + new Date(newEnd));
	...
	final void calcDates(CalculationContext context) {
		...
		if (external || task.isPinned()) { // external
