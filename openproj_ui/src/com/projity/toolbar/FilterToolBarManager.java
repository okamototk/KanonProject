...
public class FilterToolBarManager implements MenuActionConstants{
	...
	private static final int BETWEEN_SEPARATOR_SIZE = Environment.isNoPodServer() ? 8: 16;
	private static final int AFTER_SEPARATOR_SIZE = Environment.isNoPodServer() ? 10: 20;
	private static final int COMBOBOX_HEIGHT = Environment.isNoPodServer() ? 22: 28;
...
//		groupersComboBox.setBorder(defaultBorder);
		...
		Dimension sepDimension = new Dimension(BETWEEN_SEPARATOR_SIZE,BETWEEN_SEPARATOR_SIZE);
		separator1=Box.createRigidArea(sepDimension);
		separator2=Box.createRigidArea(sepDimension);
		separator3=Box.createRigidArea(new Dimension(AFTER_SEPARATOR_SIZE,AFTER_SEPARATOR_SIZE));
	...
	private void setComboSize(TransformComboBox combo){
		Dimension size = new Dimension(150,COMBOBOX_HEIGHT);
